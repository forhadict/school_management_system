...
class Material_model extends CI_Model
{
    ...
    {
        $FilePath = $this->db->get_where('material', array('material_id' => $param2))->row()->file_name;
        if (file_exists('uploads/study_material/' . $FilePath)) {
            unlink('uploads/study_material/' . $FilePath);
        }
        $this->db->where('material_id', $param2);
