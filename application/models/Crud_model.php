...
class Crud_model extends CI_Model
{
    ...
    function insert_parent()
    {
        ...
        $check_email = $this->db->get_where('parent', array('email' => $page_data['email']))->row()->email;    // checking if email exists in database
        if ($check_email != null) {
            ...
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/parent_image/' . $parent_id . '.jpg');            // image with user ID
    ...
    {
        $ImagePath = $this->db->get_where('parent', array('parent_id' => $param2))->row()->parent_id;
        if (file_exists('uploads/parent_image/' . $ImagePath . '.jpg')) {
            unlink('uploads/parent_image/' . $ImagePath . '.jpg');
        }
    ...
    function delete_hrm($param2)
    {
        ...
        if (file_exists('uploads/hrm_image/' . $ImagePath . '.jpg')) {
            unlink('uploads/hrml_image/' . $ImagePath . '.jpg');
        }
