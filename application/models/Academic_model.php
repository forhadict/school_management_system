...
class Academic_model extends CI_Model
{
    ...
    function deleteAcademicSyllabus($param2)
    {
        ...
        $FilePath = $this->db->get_where('academic_syllabus', array('academic_syllabus_code' => $param2))->row()->file_name;

        if (file_exists('uploads/syllabus/' . $FilePath)) {
            unlink('uploads/syllabus/' . $FilePath);
        }
        $this->db->where('academic_syllabus_code', $param2);
