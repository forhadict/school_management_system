...
class Exam_question_model extends CI_Model
{
    ...
    {
        $FilePath = $this->db->get_where('exam_question', array('exam_question_id' => $param2))->row()->file_name;
        if (file_exists('uploads/exam_question/' . $FilePath)) {
            unlink('uploads/exam_question/' . $FilePath);
        }
        $this->db->where('exam_question_id', $param2);
